...
I configured API Gateway with an integration request and response so that when the resume webpage makes a GET request, API Gateway invokes this Lambda function and sends the result back to the client. This design demonstrated how serverless components can work together—API Gateway handling HTTP traffic and Lambda managing backend logic without the need for traditional servers.